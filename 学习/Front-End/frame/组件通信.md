...
Page({
  handler () { // 页面 .js 文件
    wx.navigateTo({
      url: '/pages/list/list',
      events: {
        currentevent: (res) => { // 接收 被打开页面发射的事件
          console.log(res)
        }
      },
      success (res) {
        res.eventChannel.emit('myevent', { name: 'tom' }) // 发送事件
      }
    })
  }
})
...
Page({
  onLoad () { // 被页面 .js 文件
    const EventChannel = this.getOpenerEventChannel() // 获取 EventChannel 对象

    EventChannel.on('myevent', (res) => { // 监听事件
      console.log(res)
    })

    EventChannel.emit('currentevent', { age: 10 }) // 向上一级页面传递数据
  }
})
```
